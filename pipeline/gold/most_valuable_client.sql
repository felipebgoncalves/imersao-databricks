...
CREATE OR REFRESH STREAMING TABLE gold.most_valuable_client
AS SELECT 
  customer_sk,
  customer_name,
  segmento,
  ...
  COUNT(CASE 
    WHEN data_hora >= (SELECT MAX(data_hora) FROM lakehouse.silver.fact_transaction_revenue) - INTERVAL 30 DAYS THEN 1 END) AS transacoes_ultimos_30_dias,
